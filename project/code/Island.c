...
int Island_State4_StartLie = 0;

int Island_Out_Point[2] = {0};
...
int Monotonicity_Change_Line_L = 0;   // 单调性转变行
...
void Continuity_Change(int start_hang, int end_hang)
{
    ...
    for (i = start_hang; i >= end_hang; i--) {
        if (abs(R_Line[i] - R_Line[i - 1]) >= 10) // 连续性阈值是5，可更改
        {
            Continuity_Change_Flag_R = i;
        }
        if (abs(L_Line[i] - L_Line[i - 1]) >= 10) // 连续性阈值是5，可更改
        {
    ...
    for (i = start; i >= end; i--) // 会读取前5后5数据，所以前面对输入范围有要求
    {
        if (R_Line[i] == R_Line[i + 5] && R_Line[i] == R_Line[i - 5] &&
            ...
            continue;
                   ...
                   R_Line[i] <= R_Line[i + 3] && R_Line[i] <= R_Line[i - 3] &&
                   R_Line[i] <= R_Line[i + 2] && R_Line[i] <= R_Line[i - 2] &&
                   R_Line[i] <= R_Line[i + 1] && R_Line[i] <= R_Line[i - 1]) { // 就很暴力，这个数据是在前5，后5中最小的，那就是单调突变点
...
void Island_State_3_Search_Line()
{
    ...
    for (lie = 7; lie <= MT9V03X_W - 11; lie++) {
...
void Image_Island_Dect()
{
    ...
    Continuity_Change(MT9V03X_H - 15, 30); // 判断赛道连续性
    if (Island_State == 0) {
        if (Island_Flag_R == 0) {
            if (Continuity_Change_Flag_R != 0 && Continuity_Change_Flag_L == 0 && Both_LostLine_Time < 10) // 如果满足右边线撕裂，左边线连续，丢线数小于阈值
            {
                Find_Right_Down_Point(MT9V03X_H - 1, 50); // 找右下角点
                /* 判定右环 */
                if (Right_Down_Line >= MT9V03X_H - 50) // 限定角点出现的位置
                {
    ...
    } else if (Island_State == 1) // 判断进入环岛后,右下方边界未消失
    {
        ...
        {
            Monotonicity_Change_Right(80, 5);
    ...
    {
        Monotonicity_Change_Right(MT9V03X_H - 5, 30);
        if (Monotonicity_Change_Line_R != 0) {
            /* 下面是连点补线 */
            // Right_Add_Line(R_Line[Monotonicity_Change_Line_R], Monotonicity_Change_Line_R,R_Line[R_Down_Point],R_Down_Point);
            /* 下面是左边界对称补线 */
        ...
        if (Boundry_Start_R >= MT9V03X_H - 2 || Monotonicity_Change_Line_R > 90) // 当圆弧靠下或者下方不丢线
        {
            Island_State = 3;
            gpio_set_level(BEEP, GPIO_HIGH);
        }
        ...
        if (Island_State3_Point[0] >= MT9V03X_H - 35 || (Island_State3_Point[1] < 26 && Island_State3_Point[1] != 0)) {
            Island_State           = 4;
        ...
        Monotonicity_Change_Left(MT9V03X_H - 10, 50);
        ...
        if (Monotonicity_Change_Line_L != 0) {
            Left_Add_Line(L_Line[Monotonicity_Change_Line_L], Monotonicity_Change_Line_L, MT9V03X_W, 0);
            if (Monotonicity_Change_Line_L >= MT9V03X_H - 20 || L_Line[Monotonicity_Change_Line_L] <= 10) {
                Island_State = 6;
                gpio_set_level(BEEP, GPIO_HIGH);
                if (Island_Number == 1) {
        ...
        /* ips200_show_int(200, 210, Island_State3_Point[1], 3);
        ips200_show_int(240, 210, Island_State3_Point[0], 3); */
    }
    /*  key_scanner();
     ...
     ips200_show_int(40, 210, Island_State, 2);
